...
1. **Read the durable assets, in order:**
   ...
   - `docs/JOHN-KNOWLEDGE.md` (John's current understanding — pitch all explanations to this level, the moving frontier; updated at every close)
...
- **Push/delivery verification** exists because push silently dropped data for hours once (139KB blob vs 64KB keepalive cap + iOS suspension) — every architectural plan assumed "push works" while it didn't. Any bundle that depends on state persisting must verify persistence actually happens, on John's real device, not just green smoke.
...
- **Update `docs/JOHN-KNOWLEDGE.md`** — append what John worked through this session: concepts now solid, terms learned, where he still gets lost. This is the moving frontier of his understanding (ground truth about what he knows, not a stale assumption). READ it at session start so technical explanations pitch at his actual current level — break down step by step, plain-English before jargon, anchor new terms to what he knows (Postman/APIs, entities, shadow tables; MAWM user exits/composite APIs/hierarchies), visuals where seeing beats reading. Stops the constant too-basic/too-advanced recalibration loop. He has strong systems judgment with a closing code-internals/vocab gap — teach to the frontier, not the floor.
