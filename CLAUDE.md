...
- **Fixture currency.** Smoke specs + walkthrough audits seed from `state-snapshot.json` at repo root. This fixture MUST reflect the most recent canonical app state. The canonical refresh mechanism is **`npm run fixture:fresh`** (Bundle 32a — pulls John's live state from worker-KV `/pull-full-state`, where the app push-on-save deposits the full `{S, BILLS}` blob 30s after every save). `npm run smoke` calls `fixture:fresh` automatically before running Playwright; use `npm run smoke:offline` only when explicitly testing without network. Bank reconciliation output (`tests/state-dump/slyght-reconciled-YYYY-MM-DD.json`) is the reset baseline when post-recon state diverges from KV. Stale fixtures produce silently-wrong test outcomes — every smoke spec passes against synthetic state that doesn't match production. Bundle 31 (2026-05-19) discovered the fixture was 5 days stale; refreshing exposed 5 latent spec dependencies that had slept under the old state.
- **State-aware ship messages.** Every CC ship message must name (a) the fixture date used for smoke/scenario verification, (b) whether `npm run fixture:fresh` ran successfully or fell back to existing fixture (with reason), (c) an impact-estimate caveat if numeric paths were touched (`getSurvivalForecast`, `BRAIN.balance`, `BRAIN.transaction`, `MODEL.*`, hero/banner numerics). Format: a short "Fixture: YYYY-MM-DD HH:MM · fresh: yes/no · numeric paths: <touched|none>" line in the morning surface or phone-verify block. Bundle 32a (2026-05-20) established this after the Pass 2 ship's "+$675 forecast impact" estimate had to be caveated against John's potentially-newer phone state.
