...
### IS-vs-SHOULD as the complete intended journey (the App Map reframe)
**What it means.** A surface's "wiring" is not a diagram of what the code *does* — it's the COMPLETE intended user journey in real terms, INCLUDING the rungs that should exist but are missing/broken. Shown as two columns (SHOULD = every rung; IS = today), the gap is the *difference* between them, marked **in position** — so the hole is seen in the context of the whole flow, and the shape of the piece that fills it is obvious.
**Proven by.** John designed this reframe (App Map brief, 2026-05-26): "the wiring is NOT what the code currently does… it's the complete intended journey INCLUDING the steps that should exist but are missing… the gap is the difference between the columns." He built the savings ladder reference himself (6 steps, missing goal-picker rung at step 4, marked in place). Specifying that the map must show the missing rung *in position* — not just "it breaks here" — is a modelling insight about representing incompleteness, and he held the prove-1-2-surfaces-first discipline before the grind.
