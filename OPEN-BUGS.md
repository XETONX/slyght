...
- **Status:** **FIXED 2026-05-26** — worker gzip-accept (commit `765d633`) + client gzip + 5s debounce + visibilitychange:hidden push + `flushSync` (cached-gzip sync keepalive) + push-outcome observability (commit `3d0215f`). gzip measured 136KB→21.8KB (34% of the 64KB keepalive cap). Full smoke 205/205. **Pending John's on-device acid test** (background a >64KB blob on the S23 → pull KV → confirm it reflects the backgrounded state) AND `wrangler deploy` of the worker FIRST. SW retry queue (true hard-close hardening) deferred to Bundle 33-cache. ADR: `docs/adr/ADR-push-reliability-gzip-debounce.md`.
