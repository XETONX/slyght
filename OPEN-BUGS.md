...
- **Status:** **FIXED + VERIFIED ON-DEVICE 2026-05-26** — worker gzip-accept (commit `765d633`) + client gzip + 5s debounce + visibilitychange:hidden push + `flushSync` (cached-gzip sync keepalive) + push-outcome observability (commit `3d0215f`). gzip measured 136KB→21.8KB (34% of the 64KB keepalive cap). Full smoke 205/205. **VERIFIED ON-DEVICE 2026-05-26** — worker deployed (version `2699f087`) → PWA reinstalled on the S23 → txns logged → app backgrounded → KV pulled: KV reflected the backgrounded state (bal $614.79, 245 txns; pushes advancing 231→241→245; last txns Spotify $15.99 + $1 Test = exactly what was logged). The backgrounding condition that used to silently drop data now delivers; `slyght_push_status` client trace confirmed. Delete-persistence also confirmed on-device: deleting the $1 Test + backgrounding → KV dropped it too (full-snapshot replace, not append — no ghost row). Both directions (create + delete) deliver through the backgrounding condition. SW retry queue (true hard-close hardening) deferred to Bundle 33-cache. ADR: `docs/adr/ADR-push-reliability-gzip-debounce.md`.
...
- **Status:** **FIXED 2026-05-26 (commit `5708da0`) — pending John's on-device done-test + deploy.** Gather refined the Drone-H spec: the SW cached NOTHING, so staleness lived in the HTTP/bfcache layer UNDER it (not a stale SW cache). Fix = update DELIVERY: network-first `cache:'reload'` for navigation/shell (online always freshest index.html) + precache shell for OFFLINE only + CACHE_VERSION prune + skipWaiting/clients.claim + `register(updateViaCache:'none')` + `reg.update()` + updatefound/controllerchange → user-controlled "Reload" banner; REMOVED the unregister-every-load scar tissue (commit 8496d12); permanent `#build-stamp` marker; sw.js itself NOT precached (stays self-updatable); fixed push icon paths. Full smoke 212/212; Guardian 4 FAILs pre-existing (zero new). Done-test: bump build-stamp → open installed PWA WITHOUT reinstall → marker updates; + airplane-mode offline load. ADR: `docs/adr/ADR-bundle-33-cache.md`. Unblocks the SW retry queue for #50's last hard-close edge.
